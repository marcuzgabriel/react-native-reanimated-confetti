...
interface UseGetShapeParams extends Pick<ConfettiProps, 'colors' | 'svgs' | 'onlyShowCustomSvgs'> {
  shapeKey: 'Circle' | 'Polygon' | 'Polyline' | 'Square' | 'Star' | 'Triangle' | 'UserSvgs';
  scale: number;
  size: number;
}
...
export const useGetConfettiShape = ({
  ...
  useMemo(() => {
    ...
    switch (shapeKey) {
      case 'Circle': {
        return <Shapes.Circle {...dimensions} fill={randomColor} scale={scale} />;
      }
      case 'Polygon': {
        return <Shapes.Polygon fill={randomColor} {...dimensions} scale={scale} />;
      }
      case 'Polyline': {
        return <Shapes.Polyline stroke={randomColor} {...dimensions} scale={scale} />;
      }
      case 'Square': {
        return <Shapes.Square fill={randomColor} {...dimensions} scale={scale} />;
      }
      case 'Star': {
        return <Shapes.Star fill={randomColor} {...dimensions} scale={scale} />;
      }
      case 'Triangle': {
        return <Shapes.Triangle fill={randomColor} {...dimensions} scale={scale} />;
      }
      case 'UserSvgs': {
        if (svgs?.length) {
          const svg = randomElementOfArray(svgs);
